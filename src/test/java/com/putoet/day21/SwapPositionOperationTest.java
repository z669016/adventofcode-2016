...
class SwapPositionOperationTest {
    ...
    void applyBackToFront() {
        final var swap = new SwapPositionOperation("swap position 3 with position 0");
        assertEquals("dbca", swap.apply("abcd"));
    }

    @Test
    void applyFrontToBack() {
        final var swap = new SwapPositionOperation("swap position 0 with position 3");
        assertEquals("dbca", swap.apply("abcd"));
    ...
    @Test
    void applyUnchanged() {
        final var swap = new SwapPositionOperation("swap position 1 with position 1");
        assertEquals("abcd", swap.apply("abcd"));
    }

    @Test
    void applyError() {
        final var swap = new SwapPositionOperation("swap position 5 with position 11");
