...
class MoveOperationTest {
    ...
    @Test
    void unApply() {
        final var move = new MoveOperation("move position 1 to position 3");
        assertEquals("abcdef", move.unApply(move.apply("abcdef")));
    }

    @Test
    void applyBackToFront() {
        final var move = new MoveOperation("move position 3 to position 0");
        assertEquals("dabc", move.apply("abcd"));
    }

    @Test
    void applyFrontToBack() {
        final var move = new MoveOperation("move position 0 to position 3");
        assertEquals("bcda", move.apply("abcd"));
    }

    @Test
    void applyUnchanged() {
        final var move = new MoveOperation("move position 1 to position 1");
        assertEquals("abcd", move.apply("abcd"));
    }

    @Test
    void applyError() {
        final var move = new MoveOperation("move position 5 to position 11");
